...
# D2Q9, fp32, A-B double buffer: 9 populations x 4 bytes x 2 buffers.
# This is the working set that must fit in the RTX 5080's 16 GB.
BYTES_PER_CELL_AB = 9 * 4 * 2
...
class Scene:
    ...
    def vram_gb(self) -> float:
        """f-population working set (A-B fp32 D2Q9) in GB — the budget."""
        return self.cells * BYTES_PER_CELL_AB / 1e9

    def report(self) -> str:
        grid = (
            f"  grid       {self.nx} x {self.ny}"
            f" = {self.cells:,} cells   (~{self.vram_gb:.3f} GB f-populations)"
        )
