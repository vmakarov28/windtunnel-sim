"""3D scene loading: YAML config -> validated Scene with resolved units.

Every 3D experiment lives in scenes3d/<name>.yaml and is defined in
physical terms. This module is the only path from a config file to
lattice parameters — everything goes through lbm.units.resolve() (the
shared, dimension-blind Reynolds triangle), so a scene that violates the
tau / u_lat guard rails cannot even be loaded. 3D scenes additionally
require `span_chars` (the periodic spanwise extent), which is also what
keeps 2D and 3D scene files from being loaded by the wrong solver.
"""
...
from lbm.units import AIR_NU, LatticeUnits, resolve

SCENES_DIR = Path(__file__).resolve().parent.parent / "scenes3d"
...
def load_scene(name: str) -> Scene:
    """Load scenes3d/<name>.yaml (or an explicit path), resolve its units."""
    path = Path(name)
    if path.suffix != ".yaml" or not path.exists():
        path = SCENES_DIR / f"{name}.yaml"
    if not path.exists():
        raise SceneError(
            f"no 3D scene named {name!r}; available: {', '.join(list_scenes())}"
        )
